    ...
    window.fg = function(fgObj){
            ...
            var error = obj.error || function(jqXHR,textStatus,errorThrown){
                try {
                    ...
                    $.each(jqXHR.responseJSON,function(){
                        var temp;
                        if (this.field) {
                            temp = this.field.split('.')[1] + ' of the payload' + (this.errorMessage ? ', ' + this.errorMessage : ' error');
                        } else {
        ...
        */
        self.evaluateVehicle = function(pobj){
            ...
            var datVehicleContainer = pobj.datVehicleContainer;
            if (typeof datVehicleContainer == 'undefined') {
                paramMissingHint.push('"datVehicleContainer" of type "object"');
                datVehicleContainer = {};
            }
            if (paramMissingHint.length > 0) {
            ...
            var obj = {
                ...
                urlParam: criteria,
                data: JSON.stringify(valuatedVehicle),
